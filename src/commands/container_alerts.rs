...
pub enum Action {
    ...
    /// Get container alert details
    Get {
        /// Container alert ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/container-security/queries/alerts/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/container-security/entities/alerts/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
