...
pub enum Action {
    /// Query container image compliance IDs
    List {
    ...
    /// Get container image compliance details
    Get {
        /// Container image compliance ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/container-compliance/queries/compliance/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!(
                "/container-compliance/entities/compliance/v1?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
