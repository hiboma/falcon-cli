...
pub enum Action {
    ...
    /// Get container package details
    Get {
        /// Container package ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/container-security/queries/packages/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!("/container-security/entities/packages/v1?{}", ids.join("&"));
            client.get(&path).await
        }
    }
}
