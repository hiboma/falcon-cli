...
pub enum Action {
    /// Query container vulnerability IDs
    List {
    ...
    /// Get container vulnerability details
    Get {
        /// Container vulnerability ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/container-security/queries/vulnerabilities/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!(
                "/container-security/entities/vulnerabilities/v1?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
