...
pub enum Action {
    /// Query Falcon container image IDs
    List {
    ...
    /// Get Falcon container image details
    Get {
        /// Falcon container image ID(s)
    ...
    match action {
        Action::List {
            ...
            let path = build_query_path(
                "/container-security/queries/falcon-images/v1",
                filter.as_deref(),
                limit,
                offset.as_deref(),
            );
        ...
        Action::Get { id } => {
            ...
            let path = format!(
                "/container-security/entities/falcon-images/v1?{}",
                ids.join("&")
            );
            client.get(&path).await
        }
